...
toCell html s  = html borderStyle [text s]
dataToCell     = toCell td
headerToCell   = toCell th
arrayToTr f s  = List.map f s |> tr borderStyle

datapointToCell: (t -> String) -> Datapoint t -> Html msg
datapointToCell toString datapoint =
  case datapoint of
    Undefined -> dataToCell "loading"
    Expired s -> dataToCell (toString s)
    Loaded s  -> dataToCell (toString s)
    Loading s -> dataToCell (toString s)


type alias Display msg = TopicInfo -> Html msg

tableFormatter : List (String, Display msg)
tableFormatter = [ ("topic",              \topic -> (topic.name |> Loaded)  |> datapointToCell (\(TopicName name)          -> name))
...
headerLine = arrayToTr headerToCell <| (List.map (\(header, _) -> header)) tableFormatter
dataLine topic = arrayToTr (\(_, display) -> (display topic)) tableFormatter

tableHtml: List TopicInfo -> Html msg
tableHtml topics = table borderStyle <| List.concat [ [headerLine] , List.map dataLine topics]
