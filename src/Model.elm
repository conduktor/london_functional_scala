...
type alias TopicInfo = {name: String, sizeInByte: Datapoint Int, partitionCount: Datapoint Int, recordCount: Datapoint Int, spread: Datapoint Int, replicationFactor: Datapoint Int}
...
applyUpdateToTopicsInfo name update infos = List.map (\topicInfo -> if (TopicName topicInfo.name == name) then update topicInfo else topicInfo) infos
