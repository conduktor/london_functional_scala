...
import Eventsource.HttpRequests as HttpRequests
import Eventsource.HttpRequests exposing (Error, Msg(..), Payload(..), decodeInfo)
...
update msg model =
  case (msg, model) of
    (HttpMessage (HttpRequests.Msg (Err _)), _) ->
        let _ = Debug.log "json decode failure" in
    ...
    (HttpMessage (HttpRequests.Msg (Ok ((GotNames _)))), _) ->
        (Failure "received GotNames while not being loading names", Cmd.none) -- FIXME should work
    ...
    (HttpMessage (HttpRequests.Msg (Ok ((GotSize _ _)))), LoadingNames) ->
        (Failure "other error", Cmd.none) -- FIXME error case
    ...
    (HttpMessage (HttpRequests.Msg (Ok ((Complete)))), _) ->
       (model, Cmd.none) -- FIXME error case


    ( HttpMessage _, Failure _ ) ->
        (Failure "other error", Cmd.none)

    (SubscriptionMessage text, state) -> let _ = Debug.log "subscription " text in (state, Cmd.none)


-- SUBSCRIPTIONS
subscriptions : Model -> Sub Msg
subscriptions model = Sub.map HttpMessage (messageReceiver decodeInfo)
