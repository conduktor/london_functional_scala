   ...
   .unwrap(for {
     ...
     inputs
       .mapAccumZIO(State.empty) { (state, input) =>
         val (newState, diff) = applyInput(state, input)
         val requests         = nextRequests(newState, diff)
         val infos            = toInfo(newState, diff)
         executeRequests(responsesQueue)(requests).as((newState, infos))
       }
   })
----
